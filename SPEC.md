...
### 型D：画像1枚型（スクリーンショットに複数店舗が入っている）
PDFではなく、スマホアプリ等で作成したスクリーンショット1枚に複数店舗の内容が詰まっている形式。

- 画像ファイルは `page_1.jpg` の1枚のみ
- すべての stores エントリが `"page": 1` を指す
- `storeCount` に実際の訪問店舗数を設定する
- 画像の取得は `Desktop/` や `Downloads/` の最新JPG/PNGを探す
...
"storeCount": 3,
"stores": [
  { "name": "イオン府中店",    "address": "広島県府中市",         "page": 1 },
  { "name": "フジ三篠店",      "address": "広島県広島市西区三篠", "page": 1 },
  { "name": "フレスタ上天満店", "address": "広島県広島市中区",    "page": 1 }
]
```
...
> |---|---|---|---|
> | PDF | 複数 | 各ページ＝別々の店舗 | A |
> | PDF | 複数 | 各ページ＝同じ店舗の別ゾーン | B |
> | PDF | 複数 | 各ページ＝複数店舗をまたぐテーマ | C |
> | 画像1枚 | 1枚 | 1枚に複数店舗がまとめて記載 | D |
